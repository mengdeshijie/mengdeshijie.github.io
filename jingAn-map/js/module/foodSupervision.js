  ...
  fnAjax() {
    $.ajax({
      ...
      success: (res) => {
        ...
        const fnDate = this.fnDate();
        setTimeout(function () {
          fnChart.fnBarY2("foodSupervision_bar1", res.bar6.data, fnDate, '告警数分析');
          fnChart.fnPie1("foodSupervision_bar2", res.bar4.data, fnDate, '告警企业分析');
          fnChart.fnPie1("foodSupervision_bar4", res.foodSupervision_bar4.data, fnDate, '近一周', "#fff");
          fnChart.fnBar("foodSupervision_bar3", res.foodSupervision_bar3.data, res.foodSupervision_bar3.yData, res.foodSupervision_bar3
            .yMax, '告警因素分析');
        }, 1000);
      }
    });
  ...
  fnEquipmentTr(selectId) {
    if (selectId == "all") {
      ...
      for (let i = 0; i < data.length; i++) {
        tr +=
          `<tr data-id="${data[i].id}"><td><div class="normal_num">${i + 1}</div></td><td>${data[i].compName}</td><td><span class="iconfont icon-weizhi"></span></td></tr>`;
      }
      $("#foodTbody").html(tr);
      ...
      publicObj.fnAjaxFloor("system/cesbasenodefood/" + selectId, (d) => {
        ...
        for (let i = 0; i < arrs.length; i++) {
          tr +=
            `<tr data-id="${arrs[i].id}"><td><div class="normal_num">${i+1}</div></td><td>${arrs[i].compName}</td><td><span class="iconfont icon-weizhi"></span></td></tr>`
        }
        $("#foodTbody").html(tr);
  ...
  fnLeftBtnHtml() {
    ...
    fnHtml({
      ...
      fn: () => {
        ...
        this.videoIds = document.querySelectorAll(".videoAn");
      }
    });
  },
  fnClick() {
